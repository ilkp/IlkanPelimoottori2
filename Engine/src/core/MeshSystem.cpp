...
namespace idop
{
	void MeshSystem::Reserve(uint32_t entityId)
	{
		...
		{
			it = _componentData.insert({ entityId & INDEX_BITS_SEQ, MeshData() }).first;
	...
	void MeshSystem::Identity(uint32_t entityId)
	{
		...
		if ((*it).second._vertices[compIndex])
		{
			delete[]((*it).second._vertices[compIndex]);
			delete[]((*it).second._triangles[compIndex]);
			delete[]((*it).second._uvs[compIndex]);
		}
		(*it).second._vertices[compIndex] = nullptr;
		(*it).second._triangles[compIndex] = nullptr;
		(*it).second._uvs[compIndex] = nullptr;
		(*it).second._verticesLength[compIndex] = 0;
		(*it).second._trianglesLength[compIndex] = 0;
		(*it).second._uvsLength[compIndex] = 0;
		(*it).second._bounds[compIndex]._min.x = 0.0f;
		(*it).second._bounds[compIndex]._min.y = 0.0f;
		(*it).second._bounds[compIndex]._min.z = 0.0f;
		(*it).second._bounds[compIndex]._max.x = 0.0f;
		(*it).second._bounds[compIndex]._max.y = 0.0f;
		(*it).second._bounds[compIndex]._max.z = 0.0f;
	}
	...
	{
		std::unordered_map<uint32_t, MeshData>::iterator it = _componentData.insert({ entityId & INDEX_BITS_SEQ, MeshData() }).first;
		(*it).second.Allocate(INDEX_BITS_COMP);
		(*it).second._reserved[entityId & INDEX_BITS_COMP] = true;
		return it;
	}
