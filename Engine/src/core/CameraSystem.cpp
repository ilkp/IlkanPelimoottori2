...
namespace idop
{
	void CameraSystem::Reserve(uint32_t entityId)
	{
		...
		{
			it = _componentData.insert(std::make_pair(entityId & INDEX_BITS_SEQ, CameraData())).first;
	...
	{
		auto it = _componentData.insert(std::make_pair(entityId & INDEX_BITS_SEQ, CameraData())).first;
		it->second.Allocate(INDEX_BITS_COMP + 1);
		it->second._reserved[entityId & INDEX_BITS_COMP] = true;
		return it;
	}
